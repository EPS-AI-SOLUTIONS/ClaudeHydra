  ...
  try {
    ...
    switch (name) {
      ...
      case 'ollama_generate': {
        ...
        // Direct call to Ollama (queue causes issues with MCP server restarts)
        const response = await generate(resolved.model, prompt, {
          temperature: safeArgs.temperature || 0.3,
          maxTokens: safeArgs.maxTokens || 2048
        });
      ...
      case 'ollama_batch': {
        ...
        // Process batch with direct Ollama calls (queue doesn't persist across MCP restarts)
        const batchResults = await Promise.all(
          prompts.map(async (prompt) => {
            try {
              ...
              return { result: response.response, error: null };
            } catch (e) {
        ...
        result = {
          results: batchResults.map((item, i) => ({
            prompt: prompts[i].substring(0, 50) + '...',
            response: item.result,
            error: item.error
          })),
          total: prompts.length,
          successful: batchResults.filter((r) => r.result).length,
