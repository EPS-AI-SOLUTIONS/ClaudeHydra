...
function parseModelId(id: string): { family: string; version: string } {
  ...
  if (legacyMatch) {
    return {
      ...
      version: `${legacyMatch[1]}.${legacyMatch[2]}`,
    };
  }
...
/** Fetch models via Next.js API proxy (no CORS issues) */
async function fetchAnthropicModels(apiKey: string): Promise<AnthropicModel[]> {
  ...
  // Static list returns already-formatted data
  if (Array.isArray(data) && data[0]?.id) {
    // Map static model list to AnthropicModel format
