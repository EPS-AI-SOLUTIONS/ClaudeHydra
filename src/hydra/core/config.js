...
const OllamaConfigSchema = ProviderConfigSchema.extend({
  ...
  models: z.object({
    router: z.string().default('llama3.2:1b'),
    researcher: z.string().default('llama3.2:3b'),
    coder: z.string().default('qwen2.5-coder:1.5b'),
    reasoner: z.string().default('phi3:mini'),
    default: z.string().default('llama3.2:3b')
  }).default({})
});
...
export class ConfigManager {
  ...
  _applyEnvOverrides(config) {
    ...
    if (process.env.HYDRA_TIMEOUT) {
      ...
      if (!isNaN(timeout)) {
        result.providers = {
          ...result.providers,
          ollama: { ...result.providers?.ollama, timeout },
    ...
    if (process.env.HYDRA_YOLO === 'true') {
      ...
      result.providers = {
        ...
        ollama: {
          ...result.providers?.ollama,
          maxRetries: 1,
          timeout: Math.min(result.providers?.ollama?.timeout || 30000, 30000)
        },
        gemini: {
  ...
  _applyMainConfigIntegration(config) {
    ...
    const mainConfigValues = {
      ...
      maxConcurrent: parseInt(process.env.QUEUE_MAX_CONCURRENT, 10) || config.providers?.ollama?.pool?.maxConcurrent,
      maxRetries: parseInt(process.env.QUEUE_MAX_RETRIES, 10) || config.providers?.ollama?.maxRetries,
      timeout: parseInt(process.env.QUEUE_TIMEOUT_MS, 10) || config.providers?.ollama?.timeout,
      ...
      // Models
      defaultModel: process.env.DEFAULT_MODEL || config.providers?.ollama?.models?.default,
      fastModel: process.env.FAST_MODEL || config.providers?.ollama?.models?.router,
      coderModel: process.env.CODER_MODEL || config.providers?.ollama?.models?.coder
    };
    ...
    if (mainConfigValues.timeout && !isNaN(mainConfigValues.timeout)) {
      result.providers = {
        ...result.providers,
        ollama: { ...result.providers?.ollama, timeout: mainConfigValues.timeout },
    ...
    if (mainConfigValues.defaultModel) {
      result.providers = {
        ...
        ollama: {
          ...result.providers?.ollama,
          models: {
            ...result.providers?.ollama?.models,
